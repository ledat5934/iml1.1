            ...
            if tuning_metrics:
                ...
                if tuning_score is not None:
                    ...
                    if os.path.exists(tuned_submission_path):
                        ...
                        logger.error(f"🚨🚨🚨 CRITICAL ERROR: submission_tuned.csv NOT FOUND! 🚨🚨🚨")
                        logger.error(f"🚨 Expected file: {tuned_submission_path}")
                        
                        # Check if wrong file was created instead
                        if os.path.exists(wrong_submission_path):
                            logger.error(f"🚨 FOUND submission.csv instead - THIS IS WRONG!")
                            logger.error(f"🚨 Hyperparameter tuning MUST create submission_tuned.csv, NOT submission.csv")
                            logger.error(f"🚨 The tuning script generated the wrong filename!")
                        else:
                            logger.error(f"🚨 No submission file found at all - tuning script failed to generate output!")
                        
                        logger.error(f"🚨 HYPERPARAMETER TUNING REQUIREMENT VIOLATED!")
                        return {"status": "failed", "error": "submission_tuned.csv file was not created - tuning script must generate submission_tuned.csv, not submission.csv"}
                else:
