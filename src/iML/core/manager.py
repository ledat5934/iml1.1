...
class Manager:
    ...
    def _log_iteration_performance_summary(self, iteration_type: str, assembler_result: Dict, hpt_result: Optional[Dict] = None):
        ...
        assembler_metrics = assembler_result.get("metrics", {})
        ...
        if hpt_result and hpt_result.get("status") == "success":
            ...
            if hpt_metrics:
                ...
                tuned_score = None
                tuning_score_priority = ['best_score', 'best_params_score', 'final_score', 'tuned_score'] + score_priority
                for score_name in tuning_score_priority:
                    if score_name in hpt_metrics:
                        tuned_score = hpt_metrics[score_name]
                        logger.info(f"🏆 Tuned primary score: {tuned_score:.4f} ({score_name})")
                        break
                
                # Compare if both scores available
                if assembler_metrics and hpt_metrics:
                    original_primary = None
                    for score_name in score_priority:
                        if score_name in assembler_metrics:
                            original_primary = assembler_metrics[score_name]
                            break
